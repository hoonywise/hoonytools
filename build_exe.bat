...
:: Use HoonyTools.pyw as the launcher (legacy launcher_gui.py removed)
...
echo Command: %PYINST_CMD% --noconfirm --windowed --onefile --name HoonyTools --icon=assets\hoonywise_gui.ico %ADDS% "%LAUNCHER%"
%PYINST_CMD% --noconfirm --windowed --onefile --name HoonyTools --icon=assets\hoonywise_gui.ico %ADDS% "%LAUNCHER%"

:: Remove the generated spec only when the build succeeded
if errorlevel 1 (
    echo PyInstaller failed — keeping HoonyTools.spec for inspection.
) else (
    del /q HoonyTools.spec >nul 2>&1
    echo Removed temporary HoonyTools.spec
)
